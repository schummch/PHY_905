...
//*********************************************************************//
const int max_iterations = 500;	        // might not be big enough to cause
                                        // over and underflow
int
main ()
{
  ...
  ofstream flows_out ("flows.out");   // open a "stream" to flows.out
  for (int i = 0; i < max_iterations; i++)
  {
    ...
    cout << i + 1 << ". under: " << under
      << " over: " << over << endl;
    flows_out << i + 1 << ". under: " << under
      << " over: " << over << endl;
  }

  flows_out.close();               // close the output stream
