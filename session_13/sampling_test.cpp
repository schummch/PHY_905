...
main (void)
{
  ...
  for (int config = 0; config < num_samples; config++)
  {
    for (int i = 0; i < num_sites; i++)  // Entire loop is one mcs
    {
      ...
      if ( (delta_energy > 0.) && (random > (exp(-delta_energy/kT))/(1.+exp(-delta_energy/kT))) )
      {
  ...
  // output the distributions of energies P(E) to data files
  // open the output file
  ostringstream my_filename;
  my_filename.str ("");
  my_filename <<"sampling_test_kT" << kT << ".dat";
