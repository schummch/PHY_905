...
main ()
{
  ...
  while (answer2 != 0)		// iterate until told to move on
      ...
      while (answer != 0)	// iterate until told to move on
	{
	  ...
	  cout << "[8] v0 = " << setprecision(5) << v0 << "\t\t";
	  cout << " [9] t_min = " << setprecision(5) << tmin << endl;
	  cout << "[10] t_mx = " << setprecision(5) << tmax << "\t";
	  cout << "[11] h = " << setprecision(5) << h << "\t\t";
    cout << "[12] b = " << setprecision(5) << b << endl;
...
rhs (double t, double y[], int i, void *params_ptr)
{
  ...
  double v0 = ((force_parameters *) params_ptr)->v0;
