...
main (void)
{
  ...
  double h1 = 0.01;		// initialize mesh spacing

  for (double t = tmin; t <= tmax; t += h1)
  {

    // find y(t+h) and output vs. t+h
    euler (N, t, y_euler, h1, rhs, params_ptr);	// Euler's algorithm

    runge4 (N, t, y_rk4, h1, rhs, params_ptr);	        // 4th order R-K

    out << scientific << setprecision (9)
      << t + h1 << "  "
      << y_euler[0] << "  "
      << y_rk4[0] << "  " << exact_answer (t + h1, params_ptr) << endl;

  }
  // out << "------------------------------------------------------" << endl;

  double h2 = 0.001;		// initialize mesh spacing

  for (double t = tmin; t <= tmax; t += h2)
  {

    // find y(t+h) and output vs. t+h
    euler (N, t, y_euler, h2, rhs, params_ptr);	// Euler's algorithm

    runge4 (N, t, y_rk4, h2, rhs, params_ptr);	        // 4th order R-K

    out << scientific << setprecision (9)
      << t + h2 << "  "
      << y_euler[0] << "  "
      << y_rk4[0] << "  " << exact_answer (t + h2, params_ptr) << endl;
  }
