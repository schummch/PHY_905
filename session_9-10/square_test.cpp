...
#define sqr(z) ((z)*(z));   //macro definition
...
main (void)
{
  ...
  // square two numbers by using squareit function
  gettimeofday (&start, NULL);     // start the clock to time the next section
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  // square two numbers using macro
  gettimeofday (&start, NULL);     // start the clock to time the next section
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  // square two numbers using inline
  gettimeofday (&start, NULL);     // start the clock to time the next section
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  cout << "final y = " << y << endl;  // prevent optimizing away loop
  gettimeofday (&stop, NULL);      // stop the clock and print elapsed time
  cout << " Evaluating " << repeat << " square(x) took "
    << elapsed (start, stop) << " seconds" << endl << endl;
